...
type Props = {
  equipment: Equipment;
  facilityId?: string;
  /** When true, show the Evaluate & Notify button (calls backend). */
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="font-mono font-bold text-foreground">{pct(probability)}</span>
      </div>
...
export function PredictionPanel({
  ...
  async function handleEvaluateAndNotify() {
    setBusy(true);
    setResult(null);
    try {
      const res = await fetch("/api/predict/evaluate", {
        ...
        body: JSON.stringify({
          ...
          facilityId: facilityId ?? equipment.facilityId,
        }),
      });
      ...
      if (!res.ok) {
        setResult({
          ...
          message:
            data?.detail ||
            data?.error ||
            "Backend unavailable. Predictions still shown from snapshot.",
        });
      } else {
        const emails =
          data?.notifications?.emails_sent ??
          data?.emails_sent ??
          0;
        ...
        setResult({
          ok: true,
          message: created
            ? `Alert created. Emails sent: ${emails}.`
            : data?.evaluation?.message ||
              `Evaluated (${data?.evaluation?.severity ?? view.level}). No new alert threshold crossed.`,
        });
      }
    } catch {
      setResult({
        ok: false,
        message:
          "Could not reach prediction service. Showing snapshot predictions only.",
      });
    } finally {
      setBusy(false);
    }
  ...
  const isCritical =
    view.level === "critical" || view.level === "high";
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Model forecasts failure risk before breakdown — with telemetry
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {busy
                ...
                : "Evaluate with AI & notify team"}
            </button>
            <p className="text-[11px] text-muted-foreground">
              Runs the backend ML evaluation. If risk crosses thresholds, a
              system alert is created and SMTP emails are sent to eligible
              roles.
            </p>
            {result && (
              <div
