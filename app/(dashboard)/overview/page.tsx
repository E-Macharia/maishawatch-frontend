  ...
  return (
    <div className="space-y-6">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 max-w-2xl text-xs sm:text-sm leading-relaxed text-muted-foreground">
                Here&apos;s the operational posture across the monitored Kenyan hospital network. Focus first on assets with the shortest predicted failure window.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:flex">
              <div className="rounded-xl border border-border bg-muted/40 px-4 py-3">
                <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Avg Risk</p>
              ...
              <div className="rounded-xl border border-border bg-muted/40 px-4 py-3">
                <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Overdue</p>
                <p className="mt-1 text-xl font-extrabold text-amber-600 dark:text-amber-400 tabular-nums">
                  {maintenanceOverdue}
                  <span className="text-xs font-normal text-muted-foreground"> assets</span>
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Reveal delay={0.04}>
          <MetricCard label="Equipment Monitored" value={metrics.equipmentCount} hint={`${metrics.facilityCount} facilities in scope`} icon={Activity} tone="blue" trend="Network coverage stable" />
        </Reveal>
        <Reveal delay={0.08}>
          <MetricCard label="Critical Assets" value={metrics.criticalCount} hint="Shortest failure windows" icon={ShieldAlert} tone="red" trend="Priority Review Required" />
        </Reveal>
        <Reveal delay={0.12}>
          <MetricCard label="Open Alerts" value={metrics.activeAlertCount} hint={`${metrics.discrepancyCount} with discrepancy flags`} icon={AlertTriangle} tone="amber" trend="Monitoring Active" />
        </Reveal>
        <Reveal delay={0.16}>
          <MetricCard label="Facilities At Risk" value={facilityRanking.filter((f) => f.averageRisk >= 60).length} hint="Average risk score ≥ 60" icon={Building2} tone="emerald" trend="Network Stable" />
        </Reveal>
      </section>
          ...
          <CardContent className="p-0">
            <div className="grid divide-y divide-border sm:grid-cols-3 sm:divide-x sm:divide-y-0">
              <div className="flex items-center gap-4 px-6 py-4">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Sensor Anomalies</p>
                  <p className="mt-1 text-lg font-bold text-foreground tabular-nums">{metrics.anomalyCount}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">Recent telemetry signals</p>
                </div>
              </div>
              <div className="flex items-center gap-4 px-6 py-4">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Recorded Downtime</p>
                  <p className="mt-1 text-lg font-bold text-foreground tabular-nums">{metrics.downtimeHours}h</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">Failure-event downtime</p>
                </div>
              </div>
              <div className="flex items-center gap-4 px-6 py-4">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Average Utilization</p>
                  <p className="mt-1 text-lg font-bold text-foreground tabular-nums">{metrics.utilization}%</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">Latest equipment telemetry</p>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <Card className="min-h-[360px] border-border bg-card shadow-xs">
            <CardHeader className="border-b border-border pb-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <CardTitle className="text-sm font-bold text-foreground">Utilization vs Reported Usage</CardTitle>
                  <p className="mt-1 text-xs text-muted-foreground">Portfolio average from current scenario telemetry.</p>
                </div>
          ...
          <Card className="min-h-[360px] border-border bg-card shadow-xs">
            <CardHeader className="border-b border-border pb-4">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">Current modeled risk by asset.</p>
              </div>
            </CardHeader>
        ...
        <Reveal delay={0.26}>
          <Card className="border-border bg-card shadow-xs">
            <CardHeader className="border-b border-border pb-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <CardTitle className="text-sm font-bold text-foreground">Priority Intervention Queue</CardTitle>
                  <p className="mt-1 text-xs text-muted-foreground">Assets ordered by predicted failure lead time.</p>
                </div>
                <Link href="/equipment" className="inline-flex items-center gap-1 text-xs font-bold text-primary hover:underline">
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 truncate text-xs text-muted-foreground">
                        {item.facilityName} • {formatEquipmentType(item.type)} • {item.serialNumber}
                      </p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="text-sm font-bold text-foreground font-mono">{item.leadTimeDays}d</p>
                      <p className="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">lead time</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <Card className="border-border bg-card shadow-xs">
            <CardHeader className="border-b border-border pb-4">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-sm font-bold text-foreground">Network Hotspots</CardTitle>
                  <p className="mt-1 text-xs text-muted-foreground">Highest average equipment risk.</p>
                </div>
              ...
              <div className="divide-y divide-border">
                {facilityRanking.map((facility, index) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-bold text-foreground">{facility.name}</p>
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        {facility.county} • {facility.equipmentCount} assets
                      </p>
                    </div>
                    <div className="text-right">
                      <p className={`text-xs font-bold font-mono ${facility.averageRisk >= 70 ? "text-red-500" : "text-amber-500"}`}>
        ...
        <Reveal delay={0.34}>
          <Card className="h-[310px] border-border bg-card shadow-xs">
            <CardHeader className="border-b border-border pb-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">Asset count by clinical area.</p>
            </CardHeader>
        ...
        <Reveal delay={0.38}>
          <Card className="h-[310px] border-border bg-card shadow-xs">
            <CardHeader className="border-b border-border pb-4">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-sm font-bold text-foreground">Live Alerts</CardTitle>
                  <p className="mt-1 text-xs text-muted-foreground">Highest urgency signals.</p>
                </div>
                <Link href="/alerts" className="text-xs font-bold text-primary hover:underline">
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-2">
                        ...
                        <span className="truncate text-xs font-bold text-foreground">{getFacilityName(alert.facilityId)}</span>
                      </div>
        ...
        <Reveal delay={0.42}>
          <Card className="h-[310px] border-primary/20 bg-gradient-to-br from-primary/10 via-card to-card shadow-xs">
            ...
            <CardContent>
              ...
              <h3 className="mt-2 text-lg font-bold tracking-tight text-foreground">Review the 9-day failure window.</h3>
              <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                Several critical assets are entering their modeled intervention window. Open the equipment queue and inspect shortest lead times.
              </p>
              <Link
                href="/equipment"
