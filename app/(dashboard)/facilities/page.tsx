...
import { ArrowUpRight, Building2, MapPin, Search, ShieldAlert, X } from "lucide-react";
...
export default function FacilitiesPage() {
  ...
  const filtered = useMemo(
    () =>
      facilities.filter((f) => {
        ...
        const level = f.averageRisk >= 75 ? "critical" : f.averageRisk >= 55 ? "high" : f.averageRisk >= 35 ? "medium" : "low";
        return (
          (!q || `${f.name} ${f.county} ${f.facilityType}`.toLowerCase().includes(q)) &&
  ...
  return (
    <div className="space-y-6">
      ...
      <Reveal>
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-primary">Network View</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Facilities Directory</h1>
          <p className="mt-1 max-w-2xl text-xs sm:text-sm text-muted-foreground">
            Compare operational risk, equipment exposure, and maintenance pressure across monitored hospitals.
          </p>
        </div>
      </Reveal>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <MetricCard label="Facilities Monitored" value={facilities.length} hint="Across monitored counties" icon={Building2} />
        <MetricCard label="Elevated Risk" value={elevated} hint="Average risk score ≥ 60" icon={ShieldAlert} tone="red" />
        <MetricCard
        ...
        <Card className="border-border bg-card shadow-xs">
          <CardHeader className="border-b border-border pb-4">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div>
                <CardTitle className="text-sm font-bold text-foreground">Facility Risk Register</CardTitle>
                <p className="mt-1 text-xs text-muted-foreground">{filtered.length} facilities match the current view.</p>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px] text-left">
                <thead className="bg-muted/40 border-b border-border">
                  <tr>
                    {["Facility", "County", "Service Level", "Assets", "Critical", "Risk", ""].map((h) => (
                    ...
                    <tr key={f.id} className="transition-colors hover:bg-accent/40">
                      <td className="px-6 py-4">
                        <Link href={`/facilities/${f.id}`}>
                          <p className="text-xs font-bold text-foreground hover:underline">{f.name}</p>
                          <p className="mt-0.5 text-[11px] text-muted-foreground">{f.facilityType}</p>
                        </Link>
                      </td>
                      <td className="px-6 py-4 text-xs font-medium text-muted-foreground">{f.county}</td>
                      <td className="px-6 py-4 text-xs capitalize text-muted-foreground">{f.serviceLevel.replace("_", " ")}</td>
                      <td className="px-6 py-4 text-xs font-mono font-bold text-foreground">{f.equipmentCount}</td>
                      <td className="px-6 py-4 text-xs font-mono font-bold text-red-500">{f.criticalCount}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <StatusBadge level={f.averageRisk >= 75 ? "critical" : f.averageRisk >= 55 ? "high" : f.averageRisk >= 35 ? "medium" : "low"} />
                          <span className="text-xs font-mono font-bold text-foreground">{f.averageRisk}</span>
                        </div>
                      </td>
